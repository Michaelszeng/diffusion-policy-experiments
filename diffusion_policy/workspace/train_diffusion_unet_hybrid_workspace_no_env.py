            ...
            if getattr(cfg.training, "checkpoint_path", None) is not None:
                ...
                if ckpt_path.is_file():
                    ...
                    if getattr(cfg.training, "reset_step_counter_on_warmstart", False):
                        # Warm-starting from a given checkpoint file (i.e. for fine-tuning).
                        # In this case, the loaded run's step/epoch bookkeeping, reset to fresh LR schedule, 
                        # interpret the given step count as additional steps for this run.
                        accelerator.print(
            ...
            for local_epoch_idx in range(resume_epoch, cfg.training.num_epochs):
                ...
                with tqdm.tqdm(
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        # Noisy trajectory construction uses no learnable parameters;
                        # it can stay outside the accumulate context.
                    ...
                    if (self.epoch % cfg.training.sample_every) == 0 and cfg.training.log_val_mse:
                        with torch.no_grad():
                            ...
                            for dataset_idx in range(self.num_datasets):
                                ...
                                val_obs_dict = {k: v for k, v in val_batch.items() if k != "action"}
