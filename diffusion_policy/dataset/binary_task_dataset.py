...
class BinaryTaskDataset(BaseImageDataset):
        ...
        for i, zarr_config in enumerate(zarr_configs):
            ...
            self.replay_buffers.append(
                ReplayBuffer.copy_from_path(
                    zarr_path=zarr_path, store=zarr.MemoryStore(), keys=keys
                )
            )
            ...
            val_mask = get_val_mask(
                n_episodes=n_episodes, val_ratio=dataset_val_ratio, seed=seed
            )
            train_mask = ~val_mask
        ...
        for replay_buffer in self.replay_buffers:
            ...
            for key in low_dim_keys:
                ...
                if key not in input_stats:
                    ...
                    input_stats[key]["max"] = torch.maximum(
                        input_stats[key]["max"], _max
                    )
                    input_stats[key]["min"] = torch.minimum(
                        input_stats[key]["min"], _min
                    )
    ...
    def _sample_to_data(self, sample, sampler_idx):
        ...
        if self.use_one_hot_encoding:
            if self.one_hot_encoding is None:
                data["one_hot_encoding"] = np.zeros(self.num_datasets).astype(
                    np.float32
                )
    ...
    def _validate_zarr_configs(self, zarr_configs):
        ...
        if num_null_sampling_weights not in [0, N]:
            raise ValueError(
                "Either all or none of the zarr_configs must have a sampling_weight"
            )
