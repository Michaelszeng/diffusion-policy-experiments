...
def crop_image_from_indices(images, crop_indices, crop_height, crop_width):
    ...
    assert torch.all(crop_indices[..., 0] >= 0)
    assert torch.all(crop_indices[..., 0] < (image_h - crop_height))
    assert torch.all(crop_indices[..., 1] >= 0)
    assert torch.all(crop_indices[..., 1] < (image_w - crop_width))
